#!/usr/bin/env python
"""Prepare or execute DD-270's thirty-second controlled vapor-holdup trajectory."""
...
SCHEMA = "dd270-core-v3-c3c4-vapor-holdup-terminal-control-thirty-second-contract-v1"
RESULT_SCHEMA = "dd270-core-v3-c3c4-vapor-holdup-terminal-control-thirty-second-result-v1"
CONTRACT = Path(
    "logs/dd270_core_v3_c3c4_vapor_holdup_terminal_control_thirty_second_contract_20260820.json"
)
RESULT = Path(
    "logs/dd270_core_v3_c3c4_vapor_holdup_terminal_control_thirty_second_20260820.json"
)
EVIDENCE = Path(
    "logs/dd270_core_v3_c3c4_vapor_holdup_terminal_control_thirty_second_20260820.npz"
)
JOURNAL = Path(
    "logs/dd270_core_v3_c3c4_vapor_holdup_terminal_control_thirty_second_journal_20260820"
)
CONTRACT_DOC = Path(
    "docs/dd_270_core_v3_c3c4_vapor_holdup_terminal_control_thirty_second_contract_20260820.md"
)
RESULT_DOC = Path(
    "docs/dd_270_core_v3_c3c4_vapor_holdup_terminal_control_thirty_second_20260820.md"
)
...
def prepare(contract_path: Path, contract_doc_path: Path) -> dict[str, Any]:
    ...
    if not source.get("pass_gate"):
        raise RuntimeError("DD-270 requires the accepted DD-269 result")
    payload: dict[str, Any] = {
        "schema_id": SCHEMA,
        "authorization": (
            "DD-269 authorizes one separately frozen extended controlled trajectory"
        ),
        "sources": {
        ...
        "trajectory": {
            "accepted_source_time_sec": 5.0,
            "source_replay_steps": 20,
            "nominal_continuation_steps": 100,
    ...
    if (ROOT / contract_path).exists() or (ROOT / contract_doc_path).exists():
        raise RuntimeError("DD-270 contract artifact already exists")
...
def _contract_markdown(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        (
            "# DD-270 Thirty-Second Controlled Trajectory Contract",
            "",
...
def _verify(payload: dict[str, Any], contract_path: Path) -> None:
    ...
    if claimed != actual or payload.get("schema_id") != SCHEMA:
        raise RuntimeError("DD-270 contract checksum or schema failed")
    for path, expected in payload["sources"].items():
        if _sha(Path(path)) != expected:
            raise RuntimeError(f"DD-270 source changed: {path}")
    for path, expected in payload["implementation_sha256"].items():
        if _sha(Path(path)) != expected:
            raise RuntimeError(f"DD-270 implementation changed: {path}")
    if any((ROOT / path).exists() for path in (RESULT, EVIDENCE, JOURNAL)):
        raise RuntimeError("DD-270 result exists; rerun is prohibited")
...
def _journal(index: int | str, time_sec: float, report: Mapping[str, Any], coordinates: np.ndarray) -> None:
    ...
    if destination.exists():
        raise RuntimeError(f"DD-270 journal collision: {destination}")
    destination.write_text(
        _json_text(
            {
                "schema_id": "dd270-controlled-endpoint-journal-v1",
                "index": index,
                "time_sec": time_sec,
                "report": report,
...
def _replay_source(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for index in range(20):
        evaluation = dd267._evaluate(
            ...
            coordinates[index],
            0.25,
            f"dd270:source_replay_{index + 1}",
            "residual",
        )
        if np.max(np.abs(evaluation.controller_memory_endpoint - memories[index])) > 1.0e-12:
            raise RuntimeError("DD-270 source controller-memory parity failed")
    ...
    if max(parity.values()) > 1.0e-10:
        raise RuntimeError("DD-270 source endpoint parity failed")
    return {
...
def execute(contract_path: Path) -> dict[str, Any]:
    ...
    for index in range(21, 121):
        ...
        if index % 4 == 0:
            print(
                f"DD-270 accepted endpoint {index}/120 "
                f"(t={time_sec:.2f} s, residual={report['scaled_residual_inf_norm']:.2e})",
                flush=True,
            )
    if branch is None:
        raise RuntimeError("DD-270 refinement branch was not captured")
    ...
    gates = {
        ...
        "one_fresh_jacobian_per_new_root": len(matrices) == 102,
        "journals_complete": len(list((ROOT / JOURNAL).glob("endpoint_*.json"))) == 102,
    ...
    report = {
        ...
        "classification": (
            "vapor_holdup_terminal_control_thirty_second_passed"
            if passed
            else "vapor_holdup_terminal_control_thirty_second_failed"
        ),
        "decision": (
