#!/usr/bin/env python
"""Audit the water-methanol stationary Jacobian at two difference steps."""
...
DEFAULT_JSON = Path(
    "logs/core_v3_water_methanol_stationary_jacobian_20260831.json"
)
DEFAULT_DOC = Path(
    "docs/core_v3_water_methanol_stationary_jacobian_20260831.md"
)
DEFAULT_MATRIX = Path(
    "logs/core_v3_water_methanol_stationary_jacobian_20260831.npz"
)
STEPS = (1.0e-5, 5.0e-6)
...
def _sentinel_columns(contract: Any, limit: int = 12) -> tuple[int, ...]:
    by_block: dict[str, list[int]] = {}
    for index, variable in enumerate(contract.variables):
        by_block.setdefault(variable.block, []).append(index)
    candidates: list[int] = []
    for indices in by_block.values():
        candidates.extend((indices[0], indices[-1]))
    unique = tuple(dict.fromkeys(candidates))
    if len(unique) <= limit:
        return unique
    chosen = np.linspace(0, len(unique) - 1, limit, dtype=int)
    return tuple(unique[int(index)] for index in chosen)
...
def execute(
    ...
    for step in STEPS:
        matrix, groups = colored_central_difference_jacobian(
            ...
            pattern=pattern,
            step=step,
            state_id=f"water_methanol:jacobian:h={step:.1e}",
        )
        if groups_reference is None:
            groups_reference = groups
        elif groups != groups_reference:
            raise RuntimeError("Jacobian coloring changed between difference steps")
    ...
    sentinel_results = []
    direct_step = STEPS[0]
    for column in _sentinel_columns(contract):
        delta = np.zeros(dimension)
        delta[column] = direct_step
        direct = (
            objective(point + delta, f"water_methanol:sentinel:{column}:plus")
            - objective(point - delta, f"water_methanol:sentinel:{column}:minus")
        ) / (2.0 * direct_step)
    ...
    report = {
        "schema_id": "core-v3-water-methanol-stationary-jacobian-v1",
        "classification": (
            "stationary_jacobian_passed" if passed else "stationary_jacobian_failed"
        ),
        "bulk_provider": "dwsim",
        ...
        "pass_gate": passed,
        "decision": (
            "authorize_one_bounded_stationary_solve"
            if passed
            else "stop_before_stationary_solve"
        ),
    }
    evidence = {
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        (
            ...
            f"- Size and color groups: `{report['dimension']} / {report['color_count']}`",
            ...
            f"- Matrix change between step sizes: `{report['matrix_relative_frobenius_change']:.6e}`",
            f"- Singular-value change: `{report['spectrum_relative_change']:.6e}`",
            f"- Direct column checks passed: `{sum(item['pass_gate'] for item in report['sentinel_columns'])}/{len(report['sentinel_columns'])}`",
...
def main() -> int:
    ...
    report, evidence = execute(args.density_model)
        ...
        json.dumps(
            {
                ...
                "condition": report["step_results"][0]["condition"],
            },
            indent=2,
        )
    )
