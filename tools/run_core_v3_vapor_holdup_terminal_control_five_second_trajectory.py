#!/usr/bin/env python
"""Prepare or execute DD-269's five-second controlled vapor-holdup trajectory."""
...
SCHEMA = "dd269-core-v3-c3c4-vapor-holdup-terminal-control-five-second-contract-v1"
RESULT_SCHEMA = "dd269-core-v3-c3c4-vapor-holdup-terminal-control-five-second-result-v1"
CONTRACT = Path(
    "logs/dd269_core_v3_c3c4_vapor_holdup_terminal_control_five_second_contract_20260820.json"
)
RESULT = Path(
    "logs/dd269_core_v3_c3c4_vapor_holdup_terminal_control_five_second_20260820.json"
)
EVIDENCE = Path(
    "logs/dd269_core_v3_c3c4_vapor_holdup_terminal_control_five_second_20260820.npz"
)
JOURNAL = Path(
    "logs/dd269_core_v3_c3c4_vapor_holdup_terminal_control_five_second_journal_20260820"
)
CONTRACT_DOC = Path(
    "docs/dd_269_core_v3_c3c4_vapor_holdup_terminal_control_five_second_contract_20260820.md"
)
RESULT_DOC = Path(
    "docs/dd_269_core_v3_c3c4_vapor_holdup_terminal_control_five_second_20260820.md"
)
SOURCE_RESULT = dd267.RESULT
SOURCE_EVIDENCE = dd267.EVIDENCE
...
def prepare(contract_path: Path, contract_doc_path: Path) -> dict[str, Any]:
    ...
    if source.get("pass_gate") or not adjudication.get("pass_gate"):
        raise RuntimeError("DD-269 requires the preserved DD-267/DD-268 decisions")
    payload: dict[str, Any] = {
        "schema_id": SCHEMA,
        "authorization": (
            "DD-268 authorizes one separately frozen longer controlled trajectory"
        ),
        "sources": {
        ...
        "trajectory": {
            "accepted_source_time_sec": 1.0,
            "source_replay_steps": 4,
            "nominal_continuation_steps": 16,
            "nominal_step_sec": 0.25,
            "nominal_final_time_sec": 5.0,
            "refinement_start_sec": 4.75,
            "refined_steps": 2,
    ...
    if (ROOT / contract_path).exists() or (ROOT / contract_doc_path).exists():
        raise RuntimeError("DD-269 contract artifact already exists")
...
def _verify(payload: dict[str, Any], contract_path: Path) -> None:
    ...
    if claimed != actual or payload.get("schema_id") != SCHEMA:
        raise RuntimeError("DD-269 contract checksum or schema failed")
    for path, expected in payload["sources"].items():
        if _sha(Path(path)) != expected:
            raise RuntimeError(f"DD-269 source changed: {path}")
    for path, expected in payload["implementation_sha256"].items():
        if _sha(Path(path)) != expected:
            raise RuntimeError(f"DD-269 implementation changed: {path}")
    if any((ROOT / path).exists() for path in (RESULT, EVIDENCE, JOURNAL)):
        raise RuntimeError("DD-269 result exists; rerun is prohibited")
...
def _journal(index: int | str, time_sec: float, report: Mapping[str, Any], coordinates: np.ndarray) -> None:
    ...
    if destination.exists():
        raise RuntimeError(f"DD-269 journal collision: {destination}")
    destination.write_text(
        _json_text(
            {
                "schema_id": "dd269-controlled-endpoint-journal-v1",
                "index": index,
                "time_sec": time_sec,
                "report": report,
...
def _replay_source(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if max(parity.values()) > 1.0e-10:
        raise RuntimeError("DD-269 source endpoint parity failed")
    return {
        ...
        "prior": final,
        "parity": parity,
        "reports": source["nominal_endpoints"],
    }
...
def execute(contract_path: Path) -> dict[str, Any]:
    ...
    for index in range(5, 21):
        ...
        if index % 4 == 0:
            print(
                f"DD-269 accepted endpoint {index}/20 "
                f"(t={time_sec:.2f} s, residual={report['scaled_residual_inf_norm']:.2e})",
                flush=True,
            )
    if branch is None:
        raise RuntimeError("DD-269 refinement branch was not captured")
    ...
    for index in range(1, 3):
        refined_coordinates, final, report, matrix = dd267._solve_endpoint(
            ...
            refined_prior,
            0.125,
            f"dd269:refined_{index}",
        )
        time_sec = 4.75 + index * 0.125
        report.update({"index": index, "time_sec": time_sec})
    ...
    nominal_response = dd267._response(
        initial_reference, nominal_evaluations, [0.25] * 20
    )
    refined_path = [*nominal_evaluations[:19], *refined_evaluations]
    refined_response = dd267._response(
        initial_reference, refined_path, [0.25] * 19 + [0.125, 0.125]
    )
    ...
    gates = {
        ...
        "nominal_complete": len(nominal_evaluations) == 20,
        ...
        "one_fresh_jacobian_per_new_root": len(matrices) == 18,
        "journals_complete": len(list((ROOT / JOURNAL).glob("endpoint_*.json"))) == 18,
    ...
    report = {
        ...
        "classification": (
            "vapor_holdup_terminal_control_five_second_passed"
            if passed
            else "vapor_holdup_terminal_control_five_second_failed"
        ),
        "decision": (
        ...
        "wall_clock_sec": wall,
        "simulation_wall_ratio": 4.0 / max(wall, 1.0e-300),
        "gates": gates,
        "pass_gate": passed,
...
def _result_markdown(report: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        (
            ...
            f"- Nominal endpoints: `{len(report['nominal_endpoints'])}` through `5.0 s`",
