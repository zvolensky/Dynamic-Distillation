...
Tabular thermo surrogate provider for faster dynamic runs.

Concept
-------
Use precomputed Peng-Robinson flash surfaces over (T, P) for one or more
reference compositions ("anchors"), then interpolate during simulation.
...
At runtime, the provider blends nearby anchors in composition space and
performs bilinear interpolation in (T, P) within each anchor.
"""
