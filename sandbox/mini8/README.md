...
- Internal liquid traffic can use `--liquid-flow-mode profile`, `--liquid-flow-mode francis`, or `--liquid-flow-mode huang-htc`.
...
- The `huang-htc` liquid mode is the first applied Huang-style hybrid step in this repo: tray liquid outflow is computed from tray liquid holdup divided by a hydraulic time constant, while pressure/vapor updates remain partitioned.
...
Huang hybrid note:
- The currently applied Huang-inspired path is intentionally partial.
- Applied now: partitioned liquid dynamics via a hydraulic time constant (`huang-htc`).
- The 20-stage main-model Huang branch now uses a corrected free-pressure tray profile instead of the invalid older top-anchored pressure workaround.
- Not yet applied: Huang's separate pressure update from vapor-phase mass balance in the larger column model.
- See `sandbox/mini8/notes/huang_hybrid_path_2026-03-20.md`.
- Latest checkpoint note: `sandbox/mini8/notes/huang_hybrid_progress_2026-03-20.md`.
- Recommended 20-stage disturbance seed:
  - `sandbox/mini8/input/distillation_column_template_20stage_huang_freep_900s_seed.xlsx`
- Obsolete 20-stage Huang seed:
  - `sandbox/mini8/input/distillation_column_template_20stage_huang_900s_seed.xlsx`
  - this older workbook came from the invalid pressure-flat Huang branch and should not be reused
