#!/usr/bin/env python
"""Audit the complete vapor-holdup Jacobian at two fixed difference steps."""
...
DEFAULT_JSON = Path("logs/dd241_core_v3_c3c4_vapor_holdup_full_jacobian_20260820.json")
DEFAULT_DOC = Path("docs/dd_241_core_v3_c3c4_vapor_holdup_full_jacobian_20260820.md")
DEFAULT_MATRIX = Path("logs/dd241_core_v3_c3c4_vapor_holdup_full_jacobian_20260820.npz")
STEPS = (1.0e-5, 5.0e-6)
...
def execute() -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    problem = dd240.build_problem()
    contract = problem["contract"]
    dimension = len(contract.derivative_variables) + len(contract.algebraic_variables)
    ...
    for step in STEPS:
        matrix, groups = colored_central_difference_jacobian(
            ...
            pattern=pattern,
            step=step,
            state_id=f"dd241:h={step:.1e}",
        )
        if groups_reference is None:
            groups_reference = groups
        elif groups != groups_reference:
            raise RuntimeError("DD-241 coloring changed between difference steps")
    ...
    sentinel_columns = (
        0,
        len(contract.component_names) * len(contract.topology.column.volume_ids),
        len(contract.derivative_variables),
        len(contract.derivative_variables) + 60,
        len(contract.derivative_variables) + 80,
        len(contract.derivative_variables) + 100,
        len(contract.derivative_variables) + 118,
        dimension - 1,
    )
    sentinel_results = []
    direct_step = STEPS[0]
    ...
    for column in sentinel_columns:
        ...
        direct = (
            objective(point + delta, f"dd241:sentinel:{column}:plus")
            - objective(point - delta, f"dd241:sentinel:{column}:minus")
        ) / (2.0 * direct_step)
    ...
    report = {
        "schema_id": "dd241-core-v3-c3c4-vapor-holdup-full-jacobian-v1",
        "classification": (
            "vapor_holdup_full_jacobian_passed"
            if passed
            else "vapor_holdup_full_jacobian_failed"
        ),
        "dimension": dimension,
        ...
        "pass_gate": passed,
        "decision": (
            "authorize_one_bounded_stationary_root_campaign"
            if passed
            else "stop_vapor_holdup_nonlinear_work"
        ),
    }
    evidence = {
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        (
            "# DD-241 Full Vapor-Holdup Jacobian",
            "",
            f"- Classification: `{report['classification']}`",
            f"- Decision: `{report['decision']}`",
            f"- Dimension/colors: `{report['dimension']} / {report['color_count']}`",
            f"- Rank at h1/h2: `{first['rank']} / {second['rank']}`",
            f"- Condition at h1/h2: `{first['condition']:.6e} / {second['condition']:.6e}`",
            f"- Spectrum relative change: `{report['spectrum_relative_change']:.6e}`",
            f"- Matrix relative change: `{report['matrix_relative_frobenius_change']:.6e}`",
            f"- Sentinel columns passed: `{sum(item['pass_gate'] for item in report['sentinel_columns'])}/{len(report['sentinel_columns'])}`",
