...
@lru_cache(maxsize=1)
def _known_runner_flags() -> set[str]:
    runner_path = SRC_DIR / "dynamic_distillation" / "dynamic_run_scaffold_v1.py"
    try:
...
def _validate_runner_argv(raw_argv: List[str]) -> None:
    known_flags = _known_runner_flags()
    if not known_flags:
        return

    for token in raw_argv:
        txt = str(token or "").strip()
        if not txt or not txt.startswith("-") or _looks_like_numeric_value(txt):
            continue
        flag = txt.split("=", 1)[0]
        if flag in known_flags:
            continue
        if flag.startswith("--"):
            raise ValueError(f"Unknown runner flag: {flag}")
        raise ValueError(f"Unknown runner flag: {flag} (did you mean `-{flag}`?)")
