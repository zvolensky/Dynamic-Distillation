"""Generic total-reboiler boundary replacement for implicit Core V3 steps."""
...
from .vapor_holdup_implicit_residual_v1 import vapor_holdup_structural_pattern
...
def _ledger(
    ...
    local_rates = tuple(
        ...
        in {"liquid_component_inventory_rate", "vapor_component_inventory_rate"}
    )
    component_count = len(contract.component_names)
    if len(row_indices) != component_count or len(local_rates) != 2 * component_count:
    ...
    return row_indices, local_rates, temperature_variables


def total_reboiler_implicit_structural_pattern(
    contract: Any, *, base_pattern: np.ndarray | None = None
) -> np.ndarray:
    """Replace bottom equilibrium dependencies with the total-reboiler boundary."""
    pattern = (
        vapor_holdup_structural_pattern(contract)
        if base_pattern is None
        else np.asarray(base_pattern, dtype=bool).copy()
    )
    size = len(contract.rows)
    if pattern.shape != (size, size):
        raise ValueError("implicit structural pattern has the wrong shape")
    row_indices, local_rates, temperature_variables = _ledger(contract)
    for row_index in row_indices[:-1]:
        pattern[row_index, :] = False
        pattern[row_index, local_rates] = True
    pattern[row_indices[-1], :] = False
    pattern[row_indices[-1], temperature_variables] = True
    return pattern


def apply_total_reboiler_implicit_boundary(
    contract: Any,
    base_evaluation: Any,
    *,
    ...
    row_indices, _local_rates, _temperature_variables = _ledger(contract)
