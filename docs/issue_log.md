...
| `DD-005` | 2026-02-17 | Distillate quality control trend was counterintuitive (reflux behavior vs purity drift). | `Open` | Controller interaction (pressure vs composition) remains problematic in dynamic runs. |
...
| `DD-011` | 2026-02-19 | Initialization parity breaks when geometry-driven liquid hydraulics overrides internal ChemSep liquid profile, causing strong nonphysical drift. | `Open (Critical)` | Parity probe showed max tray residual around 2554 lbmol/h with geometry on vs around 0.1 lbmol/h with geometry off under spec/profile. Root-cause report: `docs/dd_011_hydraulic_parity_drift_report_2026-02-19.md`. Follow-up findings (runtime simplification and sequencing scope): `docs/dd_011_hydraulic_parity_followup_2026-02-21.md`. |
